...
from Problems.problem_B import problem_B

class TestProblemBValid(unittest.TestCase):
...
class TestProblemBInvalid(unittest.TestCase):
    def test_validation_errors(self):
        cases = [
            ("records=None", lambda: problem_B(None)),
            ("empty list records", lambda: problem_B([])),
            ("empty tuple records", lambda: problem_B(())),
            ("empty generator records", lambda: problem_B(iter([]))),
            ("non-iterable records", lambda: problem_B(123)),
            ("record not dict-like", lambda: problem_B(["not a dict"])),
            ("missing field (weight)", lambda: problem_B([{"name": "Alice", "score": 80}])),
            ("invalid name (empty)", lambda: problem_B([{"name": "", "score": 80, "weight": 1}])),
            ("score out of range high", lambda: problem_B([{"name": "Alice", "score": 101, "weight": 1}])),
            ("score out of range low", lambda: problem_B([{"name": "Alice", "score": -1, "weight": 1}])),
            ("score is NaN", lambda: problem_B([{"name": "Alice", "score": float("nan"), "weight": 1}])),
            ("weight not positive", lambda: problem_B([{"name": "Alice", "score": 80, "weight": 0}])),
            ("weight is NaN", lambda: problem_B([{"name": "Alice", "score": 80, "weight": float("nan")}])),
            ("records is a string", lambda: problem_B("abc")),
        ]
